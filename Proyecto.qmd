...
A raíz de que existe esta situación a nivel nacional nos enfocaremos en encontrar que probabilidad de las mujeres embarazadas tiene un riesgo de tener una defunción fetal, de forma que se puedan tomar medidas preventivas y mitigar el riesgo.

Por lo mismo el **objetivo principal** de este estudio es:

-   Determinar si un embarazo tiene probabilidad de terminar en una defunsión fetal por medio de algoritmos de aprendizaje de máquina.

Dentro de los **objetivos especificos** se encuentra:

-   Determinar la relacion que existe entre la educación materna y la cantidad de defunsiones en Guatemala.
...
#### 4.16 ¿En que sitio de ocurrencia se producen más nacimientos?
...
Se puede observar que la mayor cantidad de nacimientos se dan en un hospital público y luego en domicilios.
...
#### 4.17 ¿Cuáles son las variables que más influyen en el resultado de un embarazo?
...
df <- select(df, -SEMGES)
df <- select(df, - TOHINM)
modelo <- rpart(TIPO ~ ., data = df, method = "class")

...
La variable por utilizar será **TIPO**, la cual indica si un parto terminó en una defunción fetal
o en un nacimiento. Debido a que la investigación de este trabaja está orientada a crear una
herramienta que permita ayudar a identificar partos potencialmente problemáticos, se decidió
usar esta variable como la respuesta. Esto permitirá saber, dadas las condiciones de una mujer
embarazada, si tiene algún riesgo.

Cabe mencionar que esta variable no existía dentro del dataset inicial, sino que a la hora de
hacer la unión entre los conjuntos de datos de nacimientos y defunciones se indicó de qué
conjunto de datos era proveniente cada observación.
...
best_model <- nnet(y ~ ., data = train_data, size = best_size, decay = best_decay)
print(best_size)
print(best_decay)

```

El tuneo de parámetros se realizó con una sola capa. Se usaron topologías de 5, 10 y 15 neuronas. Aparte, se evaluó un decay de 0.1, 0.01 y 0.001. Luego de ejecutar el procedimiento, se obtuvo que el mejor modelo usa 10 nueronas y un decay de 0.01.
...
lrn = makeLearner("classif.nnet", size = 10, decay = 0.01, maxit = 1000, trace = FALSE)
...
library(nnet)
nnet_model <- nnet(TIPO ~ ., data = train, size=10, decay=0.01, softmax = FALSE)
...
confusion_matrix <- confusionMatrix(reference=test$TIPO, data = predicciones)
confusion_matrix2 <- confusionMatrix(reference=test$TIPO, data = predicciones2)
print(confusion_matrix)
print(confusion_matrix)
```
...
Sobre la precisión de los diferentes modelos para poder predecir si el embarazo terminará siendo un aborto o si el embarazo se dara con normalidad, tenemos que el peor modelo fue la regresion logistica con un 0.67 de accuracy esto se pudo dar ya que este es un algoritmo de clasificación binaria por lo que la mejor metrica para evaluarlo no sería por medio del accuracy. Mientras que el mejor modelo fue el random forest con 0.95 de accuracy pero este es uno de los modelos que se encuentra sobre ajustado por lo que realmente no es un buen resultado para nuestra investigación, por la misma razón el modelo hace las predicciones con los patrones del dataset en especifico.
Por otro lado de los modelos que **NO** se encuentran sobre ajustados tenemos que la red neuronal obtuvo un 0.93 de accuracy el cual fue superior al 0.79 de accuracy del XGBoost, por lo que el modelo que tuvo una precisión más alta y es significativo para la investigacion es **La red neuronal**.
